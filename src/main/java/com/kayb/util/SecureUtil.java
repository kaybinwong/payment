...
//					10);
	...
	public String assymEncrypt(String tPIN, String iPan, RSAPublicKey publicKey) {
		...
		log.debug("SampleHashMap::assymEncrypt([" + tPIN + "])");
		log.debug("SampleHashMap::assymEncrypt(PIN =[" + tPIN + "])");
		...
		try {
			...
			log.debug("SampleHashMap::assymEncrypt(EncryptCardNo =[" + tEncryptPIN + "])");

			return tEncryptPIN;
		} catch (Exception e) {
			log.error("assymEncrypt error", e);
			return tPIN;
		} catch (Error e) {
			log.error("assymEncrypt error", e);
			return tPIN;
